...
import {RecipientType} from "@tutao/oxmsg/dist/types/enums"
...
o.spec("SendMailModel", function () {
	...
	o.beforeEach(function () {
		...
		when(recipientsModel.resolve(anything())).thenDo(params => {
			return {
	...
	o.spec("Adding and removing recipients", function () {
		...
		o("adding duplicate to-recipient", async function () {
			...
			model.addRecipient(RecipientField.TO, recipient, false)
			const r1 = model.getRecipient(RecipientField.TO, recipient.address)!

			model.addRecipient(RecipientField.TO, recipient, false)

			verify(recipientsModel.resolve({...recipient, resolveLazily: false}), {times: 1})
		...
		o("add different to-recipients", async function () {
			...
			model.addRecipient(RecipientField.TO, pablo, false)
			model.addRecipient(RecipientField.TO, cortez, false)

			verify(recipientsModel.resolve({...pablo, resolveLazily: false}))
			verify(recipientsModel.resolve({...cortez, resolveLazily: false}))
		...
		o("add duplicate recipients to different fields", async function () {
			...
			model.addRecipient(RecipientField.TO, recipient, false)
			model.addRecipient(RecipientField.CC, recipient, false)

			verify(recipientsModel.resolve({...recipient, resolveLazily: false}), {times: 2})
